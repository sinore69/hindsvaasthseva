...
export default function TopBar() {
  return (
    <nav className="hidden md:flex w-full text-sm font-bold justify-center top-5 z-20 fixed px-8">
      <div className="max-w-screen-lg w-full flex flex-row justify-between items-center px-6 py-4 bg-secondary rounded-lg shadow-lg">
        {/* Logo */}
        <div className="px-4">
          <Link to="/">
        ...
        <Link to={localStorage.getItem("token") ? "/profile" : "/auth"}>
          <div className="px-4 flex items-center space-x-2">
            <FaUser size={18} />
              <div>{localStorage.getItem("userEmail") ? `Hi ${localStorage.getItem("userEmail")}` : "Login/Signup"}</div>
          </div>
        </Link>
      </div>
    </nav>
